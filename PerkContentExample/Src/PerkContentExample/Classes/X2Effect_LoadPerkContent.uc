...
class X2Effect_LoadPerkContent extends X2Effect_Persistent;
...
simulated function LoadPerks(XComGameState VisualizeGameState, out VisualizationActionMetadata ActionMetadata) {
    ...
    if(UnitState == none) {
        `RedScreen("Warning, was unable to find a UnitState for X2Effect_LoadPerkContent!");
        return;
    }

    UnitPawnNativeBase = XGUnit(UnitState.GetVisualizer()).GetPawn();
    if(UnitPawnNativeBase == none) {
        `RedScreen("Warning, was unable to find a UnitPawnNativeBase for X2Effect_LoadPerkContent!");
        return;
    }
